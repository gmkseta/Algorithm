	...
	*************************/
	...
	{	//ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
		...
		{ /* event 1 : customer arrival */
			...
			s += n*(elapsed_time - tn); //elapsed_time - tn 은 지난 시간이고 n 은 그동안 안에 있던 사람 수 임 .
			n++;//n 증가시켜줌
			tn = elapsed_time;//지금으로 시점 옮겨주고~

			next_arrival_time = elapsed_time + expntl(Ta);//다음 도착시간 계산

			if (n == 1) //아,... n++ 이니까 n이 0일때만 일러오는거구나.. 그냥 시작이구나....ㅇㅎ...
			{//고객 한명일 때
				tb = elapsed_time; //tb에 경과시간 넣어주고 아마 출발이 더 빠르면 n이 0이 되고 흠 아래 에서..흠..
				next_departure_time = elapsed_time + expntl(Ts); //다음 출발시간 ( 서비스 하고나서 출발)
			}
		}
		//ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
